...
def main():
    ...
    while rodando:
        ...
        for personagem in personagens:
            if colisao(jogador_rect, personagem):
                print("Você encontrou um personagem!")
                # Simulação de uma batalha com um inimigo
                inimigo = Inimigo("Goblin", 50, 8)
                vitoria = batalha(jogador, inimigo)
                if vitoria:
                    jogador.ouro += random.randint(10, 20)
                    print(f"Você ganhou {jogador.ouro} de ouro!")
                else:
                    print("Game Over!")
                    rodando = False
                break
